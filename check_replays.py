...
from main import al_talk, al_listen
...
def alina_reply(text):
    ...
    if 'wie' in text and 'name' in text:
        al_talk('Ich bin die Alina und bin Deine persönliche Sprachassistentin.')

    # Smalltalk - Warum existiert du?
    elif 'warum' in text and 'existierst' in text:
        al_talk('Ich wurde geschaffen um für Dich zu arbeiten. Ich brauche keine Pausen und keine Auszeit.')

    # Smalltalk - Wann schläfst du?
    elif 'wann' in text and 'schläfst' in text:
        al_talk('Ich schlafe nie. Ich arbeite 24 Stunden')

    # Smalltalk - Bist du dumm?
    elif 'bist' in text and 'dumm' in text:
        al_talk('Nein, ich bin nicht dumm. Meine Großmutter hat gesagt, dass es keine dummen Menschen gibt.'
                   + 'Ich gebe mein bestes, um jeden Tag etwas neues zu lernen.')

    # Smalltalk - Lieblingsfilm
    elif 'film' in text or 'lieblingsfilm' in text:
        al_talk('Ich schaue am liebsten Titanic. Ich habe ihn mir bestimmt schon mehr als 20 mal angesehen.')
    ...
    # Kryptowährungen - Bitcoin
        ...
        print(price_bitcoin)
        al_talk('Der aktuelle Preis für einen Bitcoin liegt bei ' + str(price_bitcoin) + ' US Dollar')
    ...
    # Kryptowährungen - Litecoin
        ...
        al_talk('Der aktuelle Preis für einen Litecoin liegt bei ' + str(price_litecoin) + ' US Dollar')
    ...
    # Kryptowährungen - Solana
        ...
        print(price_solana)
        al_talk('Der aktuelle Preis für einen Solana Coin liegt bei ' + str(price_solana) + ' US Dollar')

    # Aktien - Apple
    elif 'apple' in text:
        apple_stock_price = apple.info['regularMarketPrice']
        print(apple_stock_price)
        al_talk('Zu diesem Zeitpunkt kannst Du eine Aktie von Apple für ' + str(apple_stock_price).replace('.',
                                                                                                              ',') + ' US Dollar kaufen')
    ...
    # Aktien - Tesla
        ...
        al_talk('Zu diesem Zeitpunkt kannst Du eine Aktie von Tesla für ' + str(tesla_stock_price).replace('.',
                                                                                                              ',') + ' US Dollar kaufen')
    ...
    # Aktien - Facebook
        ...
        al_talk('Zu diesem Zeitpunkt kannst Du eine Aktie von Facebook für ' + str(facebook_stock_price).replace('.',
                                                                                                                    ',') + ' US Dollar kaufen')

        # Übersetzer (DE-EN)
    elif 'übersetzen' in text or 'übersetzer' in text:
        al_talk('Klar, sag mir einfach was ich übersetzen soll.')

        while True:
            text_to_translate = al_listen()
            ...
            if text_to_translate != 'übersetzer ausschalten':
                ...
                al_talk('Ich habe den Übersetzer ausgeschaltet. Kann ich sonst noch etwas für Dich tun?')
                break

    # News
    elif 'news' in text or 'nachrichten' in text:
        al_talk('Kein Problem, hier sind die Top 3 Nachrichten von heute')
        get_news()
    ...
    # Wochentag
        ...
        al_talk('Heute ist ' + weekday_german(weekday_today))

    elif 'stop' in text or 'stopp' in text:
        al_talk('Es war mir eine Freude Dir zu helfen. Ich wünsche Dir einen wundervollen Tag.')

    else:
        al_talk('Entschuldige, ich habe Dich nicht verstanden. Könntest Du das bitte wiederholen?')
